...
export function ClinicSearchScreen() {
  ...
  const loadClinics = useCallback(async (lat: number, lng: number) => {
    try {
  ...
  const requestLocationAndLoad = useCallback(async () => {
    ...
    try {
      ...
      await loadClinics(coords.lat, coords.lng);
    } catch {
  ...
  return (
    <View style={styles.container}>
      ...
      {state === 'empty' && (
        <View style={[styles.emptyOverlay, { top: insets.top + spacing.md }]}>
          <View style={styles.emptyCard}>
            <Ionicons name="search-outline" size={20} color={colors.muted} />
            <Text style={styles.emptyText}>Nenhuma clínica encontrada nas proximidades</Text>
          </View>
        </View>
      )}
